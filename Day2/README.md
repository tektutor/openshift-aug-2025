...
## Info - Red Hat Openshift Installation Process
<pre>
- Red Hat Openshift can be done in 2 ways
  1. Installer Provisioned Installation (IPI)
     - automated installation, works well in public cloud like AWS, Azure, etc.,
  2. User Provisioned Instalation (UPI)
     - preferred if you wish to customize your installtion, but this approach is for advanced users
     - for on-prem installation mostly this is preferred
     - this is the most complex approach but gives extreme customization
- To install Openshift in our training lab, I used KVM Type 1 Hypervisor
  - Created 8 Virtual Machines
    1. Helper VM ( a.k.a Bastion VM )
       - Installed Fedora 41 64-bit OS with CLI only
       - Installed DHCP Server
       - Installed TFTP Server
       - Installed Httpd Web server
       - Installed HAProxy Load Balancer
       - Copied RHCOS Kernel and OS Images to support PXE Network Installation
    2. BootStrap VM
       - Installed RHCOS 
       - The DHCP Server running on the helper VM will allocated IP address using the pre-configuration done with Mac address of each VMs
       - Once booted, this is where temporary Kubernetes Control Plane will be created by kubelet Container Agent service
       - the Kubernetes Control Plane will bootstrap the master1, master2 and master3 control planes
       - Once all 3 masters form a quoram of etcd clusters and master cluster, the temporarly Kubernetes cluster will be teared down automatically make the 3 masters as active masters
    3.  Master1, Master2 and Master3 Node VM
        - Installed RHCOS
        - Allocated 
          - 128 GB RAM
          - 500 GB HDD Storage
    ...
    4.  Worker1, Worker2 and Worker3 Node VMs
        - Installed RHCOS
        - Allocated 
          - 128 GB RAM
          - 500 GB HDD Storage
          - 16 CPU Cores
        - These Nodes join the cluster with the join token generated by Master Nodes 
        - The DHCP Server running on the helper VM will allocated IP address using the pre-configuration done with Mac address of each VMs
</pre>  
